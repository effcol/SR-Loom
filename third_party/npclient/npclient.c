/*
 * npclient.c -- NPClient.dll implementation, embedded into SR Loom.
 *
 * MIT License. Original author: uglyDwarf (Linuxtrack project,
 * https://github.com/uglyDwarf/linuxtrack, MIT). Subsequently
 * vendored by the OpenTrack project at contrib/npclient
 * (https://github.com/opentrack/opentrack). Bundled here verbatim
 * apart from this header.
 *
 * This DLL implements the NaturalPoint NPClient API that TrackIR-
 * compatible games load to receive head-pose data. It opens the
 * "FT_SharedMem" Win32 file mapping that SR Loom writes to (the
 * same shared memory FreeTrack-aware games already consume) and
 * serves the data back via NP_GetData with the TIR5 checksum +
 * optional XOR encryption a game requests via its GameId table.
 *
 * SR Loom extracts the built DLL into %LOCALAPPDATA%\SRLoom\NPClient
 * on first launch and points the NaturalPoint "NPClient Location"
 * registry key at that directory (saving any previous value so we
 * can defer to an existing OpenTrack install when appropriate).
 */
